...
if (function_exists('cs_framework_init')) {
  ...
  require_once( BARRISTAR_CS_FRAMEWORK . '/theme-options.php' );
  require_once( BARRISTAR_CS_FRAMEWORK . '/theme-customizer.php' );
  require_once( BARRISTAR_CS_FRAMEWORK . '/theme-metabox.php' );
   ...
   */
  if( ! function_exists( 'barristar_custom_font_upload' ) ) {
    function barristar_custom_font_upload( $db_value ) {
    ...
    add_action( 'cs_typography_family', 'barristar_custom_font_upload' );
  }
 ...
 * Check Custom Font
 */
if ( ! function_exists( 'barristar_custom_upload_font' ) ) {
  function barristar_custom_upload_font( $font ) {
 ...
 */
if ( ! function_exists( 'barristar_registered_sidebars' ) ) {
  function barristar_registered_sidebars() {
 ...
 */
if ( ! function_exists( 'barristar_typography_fonts' ) ) {
  function barristar_typography_fonts() {
    ...
    if ( ! empty( $embed_fonts ) ) {
      ...
      wp_enqueue_style( 'barristar-typography-fonts', esc_url( add_query_arg( 'family', urlencode( implode( '|', $query_args ) ) . $subsets, '//fonts.googleapis.com/css' ) ), array(), null );
    }
...
/* Typography */
if ( ! function_exists( 'barristar_get_typography' ) ) {
  function barristar_get_typography() {
    ...
    if ( ! empty( $typography ) ) {
      foreach ( $typography as $font ) {
        if ( ! empty( $font['selector'] ) ) {
          ...
          $weight  = ( $font['font']['variant'] != 'regular' ) ? barristar_esc_string( $font['font']['variant'] ) : 400;
          $style   = barristar_esc_number( $font['font']['variant'] );
          ...
          $output .= ( ! empty( $font['size'] ) ) ? 'font-size: '. barristar_check_px( $font['size'] ) .';' : '';
